...
<section class="bg-dark">
  ...
  <div class="dropdown-menu scrollable-dropdown" aria-labelledby="dropdownMenuLink">
    ...
    <a class="dropdown-item" href="{{url('/marketmoderator/usedcarparts/country/'.$country->name.'')}}">{{$country->name}}</a>
    @endforeach
  </div>
  </div>
</section>
...
<form action="{{url('/marketmoderator/ucpbyday')}}" method="post">
{{csrf_field()}}
<span> Choose day :  </span><input type="date" name="date">
<button type="submit" class="btn btn-primary"> Filter </button>
</form>
<!-- Articles of some month -->
<form action="{{url('/marketmoderator/ucpbymonth')}}" method="post">
{{csrf_field()}}
...
<form action="{{url('/marketmoderator/ucpbyyear')}}" method="post">
{{csrf_field()}}
